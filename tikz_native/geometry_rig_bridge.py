...
GEOMETRY_RIG_BRIDGE_REQUEST_SCHEMA = (
    "tikz-native-geometry-rig-bridge.request/v1"
)
GEOMETRY_RIG_BRIDGE_RESPONSE_SCHEMA = (
    "tikz-native-geometry-rig-bridge.response/v1"
)
GEOMETRY_RIG_BRIDGE_OPERATION = "analyze_geometry_rig"


def _bridge_provider_info() -> dict[str, Any]:
    info = dict(provider_info())
    capabilities = dict(info.get("capabilities", {}))
    capabilities["analyze_geometry_rig_2d"] = True
    info["capabilities"] = capabilities
    info["geometry_rig_request_schema"] = GEOMETRY_RIG_BRIDGE_REQUEST_SCHEMA
    info["geometry_rig_response_schema"] = GEOMETRY_RIG_BRIDGE_RESPONSE_SCHEMA
    return info
...
def _validate_string_array(value: object, field: str) -> None:
...
def _validate_selection(selection: Mapping[str, Any]) -> None:
    ...
    for key in ("candidate_id", "pivot"):
        if key in selection:
            _string(selection, key, field=f"selection.{key}")
    if "range" in selection:
        raw_range = selection["range"]
        if (
    ...
    for key in ("include_object_ids", "exclude_object_ids"):
        if key in selection:
            _validate_string_array(selection[key], f"selection.{key}")
...
def _validate_request(request: Mapping[str, Any]) -> None:
    ...
    if request.get("schema") != GEOMETRY_RIG_BRIDGE_REQUEST_SCHEMA:
        raise TikzNativeProviderError(
            ERROR_INPUT,
            "validate_request",
            f"request schema must be {GEOMETRY_RIG_BRIDGE_REQUEST_SCHEMA!r}",
            details={"actual": request.get("schema")},
        )
    if request.get("operation") != GEOMETRY_RIG_BRIDGE_OPERATION:
        raise TikzNativeProviderError(
            ERROR_INPUT,
            "validate_request",
            f"operation must be {GEOMETRY_RIG_BRIDGE_OPERATION!r}",
        )
    ...
    _string(input_payload, "active_object_id", field="input.active_object_id")
    _string(
        input_payload,
        "expected_asset_provider_revision",
...
def _response_operation(value: object) -> str:
    return (
        ...
        if value == GEOMETRY_RIG_BRIDGE_OPERATION
        else "unknown"
    )
...
def _error_response(
    ...
    response: dict[str, Any] = {
        "schema": GEOMETRY_RIG_BRIDGE_RESPONSE_SCHEMA,
        "ok": False,
        "operation": operation,
...
def _selected_picture(document: Any, picture_index: int):
    for picture in document.pictures:
        if picture.index == picture_index:
            return picture
    raise TikzNativeProviderError(
        ERROR_INPUT,
        "select_picture",
        ...
        details={
            "available_picture_indices": [
                picture.index for picture in document.pictures
            ]
        },
    )


def execute_geometry_rig_request(request: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            picture = _selected_picture(
                document,
                int(input_payload["picture_index"]),
            )
            rig = analyze_geometry_rig(
                picture,
                str(input_payload["active_object_id"]),
                selection=request.get("selection"),
            )
            result = attach_geometry_rig_identity(
                rig,
                source_sha256=expected_hash,
                provider_revision=str(_bridge_provider_info()["revision"]),
                expected_asset_provider_revision=str(
                    input_payload["expected_asset_provider_revision"]
                ),
            )
        except GeometryRigError as error:
            raise TikzNativeProviderError(
                ERROR_INPUT,
                "analyze_geometry_rig",
                str(error),
            ...
            raise TikzNativeProviderError(
                ERROR_PROVIDER,
                "compile_source",
                f"TikZ-native geometry analysis failed: {error}",
                details={"exception_type": type(error).__name__},
            ) from error
        return {
            "schema": GEOMETRY_RIG_BRIDGE_RESPONSE_SCHEMA,
        ...
        wrapped = TikzNativeProviderError(
            ERROR_PROVIDER,
            "geometry_rig_bridge",
            f"geometry rig bridge request failed: {error}",
            details={"exception_type": type(error).__name__},
        )
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Read-only JSON bridge for TikZ-native geometry rig analysis."
    )
    ...
    with redirect_stdout(sys.stderr):
        payload = execute_geometry_rig_request(request)
    _emit(payload, args.response)
