...
class DandelinVisibilityTests(unittest.TestCase):
    def test_existing_quadric_kernel_certifies_hidden_line_frame(self) -> None:
        ...
        self.assertEqual(
            frame.compositing_frame.sources,
            tuple(item.source for item in frame.strokes),
        )
        self.assertGreater(len(frame.compositing_frame.crossings), 0)
        self.assertEqual(
            set(frame.compositing_frame.draw_order),
            {
                *frame.compositing_frame.parent_item_ids,
                *(
                    item.item_id
                    for item in frame.compositing_frame.fragments
                    if item.painted
                ),
            },
        )
        self.assertTrue(
            any(
                item.role == "plane_boundary" and item.hidden_span_count
                for item in frame.strokes
            )
        )
        self.assertTrue(
            any(
    ...
    def test_tangent_evidence_and_directrix_boundary_fail_closed(self) -> None:
        ...
        with self.assertRaisesRegex(
            DandelinVisibilityError,
            "directrix or plane-boundary visibility requires.*finite display patch",
        ):
