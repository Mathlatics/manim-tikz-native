...
def compute_quadric_boundary_crossings(
    ...
    *,
    ...
    Private nested-silhouette certificates similarly identify three curves
    meeting tangentially at one world point.  Those pairs add no painter edge
    and are omitted instead of manufacturing tolerance-sized fragments.
    """
    ...
    unknown_certified_sources = sorted(set(certified) - set(source_ids))
    if unknown_certified_sources:
        raise QuadricBoundaryCompositingError(
            "certified support tangencies reference unknown boundary sources: "
            + ", ".join(unknown_certified_sources)
        )
    certified_pair_id_set: set[tuple[str, str]] = set()
    for source_id, raw_items in sorted(certified.items()):
        items = tuple(raw_items)
        if (
            len(items) != 2
            or not all(
                isinstance(item, _NestedSilhouetteTangencyCertificate)
                and item.curve_id == source_id
                for item in items
            )
            or len({item.crossing_id for item in items}) != 2
            or len({item.witness_curve_id for item in items}) != 2
            or len({item.contact_curve_id for item in items}) != 1
            or len({item.surface_id for item in items}) != 1
        ):
            raise QuadricBoundaryCompositingError(
                "nested silhouette pair suppression requires two certified "
                "support tangencies"
            )
        for item in items:
            triple = tuple(
            ...
            if len(triple) != 3 or not set(triple).issubset(source_ids):
                raise QuadricBoundaryCompositingError(
                    "nested silhouette pair suppression references unknown "
                    "or repeated sources"
                )
            certified_pair_id_set.update(
                {
                    (triple[0], triple[1]),
                    (triple[0], triple[2]),
                    (triple[1], triple[2]),
    ...
    for first, second in combinations(source_items, 2):
        if first.source_id in cached_ids and second.source_id in cached_ids:
            continue
        if (first.source_id, second.source_id) in certified_pair_id_set:
            continue
        ...
        for farther in far:
            for nearer in near:
                ...
                if (
                    ...
                    and BoundarySourceKind.PLANE_PATCH_EDGE in crossing_kinds
                ):
                    # A diagrammatic hidden stroke crossing the finite plane
                    # outline is an explicit teaching overlay. A hidden plane
                    # edge owns the top stroke at that crossing even when the
                    # other source is hidden too. This matters when a section
                    # edge meets both terminal rims of a frustum at the same
                    # screen point: reversing only the visible/hidden pair and
                    # retaining the hidden/hidden physical edge forms a false
                    # three-source cycle. Crossings between ordinary entity
                    # boundaries retain their certified far-to-near order.
                    plane_fragment = (
                        farther
                        if source_map[farther.source_id].source_kind
                        is BoundarySourceKind.PLANE_PATCH_EDGE
                        else nearer
                    )
                    other_fragment = (
                        nearer if plane_fragment is farther else farther
                    )
                    if (
                        plane_fragment.effective_visibility_kind
                        is VisibilityKind.HIDDEN
                    ):
                        relations.append(
                            QuadricPaintRelation(
                                other_fragment.item_id,
                                plane_fragment.item_id,
                                "diagrammatic_hidden_plane_outline_crossing_overlay:"
                                f"{crossing.crossing_id}",
                            )
                        )
                        continue
                    if (
                        farther.effective_visibility_kind
                        ...
                        visible = nearer if hidden is farther else farther
                        relations.append(
                            QuadricPaintRelation(
                                visible.item_id,
                                hidden.item_id,
