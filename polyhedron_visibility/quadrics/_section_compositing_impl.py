...
def quadric_plane_fragment_contours(
    ...
    screen_origin = np.asarray(frame.base_frame.visibility.projection_matrix, dtype=float)[:2] @ np.asarray(
        frame.plane.point,
        dtype=float,
    )
    screen_basis = np.column_stack(
        (
            np.asarray(frame.base_frame.visibility.projection_matrix, dtype=float)[:2]
            @ plane_u,
            np.asarray(frame.base_frame.visibility.projection_matrix, dtype=float)[:2]
            @ plane_v,
        )
...
def compute_quadric_section_compositing(
    ...
    *,
    ...
    relations = _relation_chain(items.ordered)
    active_curve_ids = {
