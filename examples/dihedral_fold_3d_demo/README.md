...
- `scene.py`：只组装原生对象并播放 timeline，不再画第二份图。
...
- `DrawSpacePlaneInteraction` 会保存为明确的线—面遮挡关系。
...
编译器会把它保存为 `point_on_segment`：`M` 始终位于
...
编译器会把它保存为 `project_point_to_line`：`N` 始终是 `M` 在直线
...
归并为稳定的 `semanticGroups`，调用方无需拆解 `objectId`。

相机切换不写在 TikZ 中。TikZ 只保存入口投影视角；`front`、`side`、
`top`、`oblique`、`isometric` 等相机操作由 Geometry Rig 时间线选择。
v1 固定使用 `end_policy: restore_entry`：片段结束时恢复入口几何和入口相机。
...
找到 TikZ 和 motion 配置，因此仓库复制到其他位置后仍可运行。

## 这一版的边界

- 遮挡仍是平行投影下的有限三角形/平行四边形线—面关系。
- 折叠驱动已支持任意命名空间轴，但这个夹具特意使用易于核对的 `AB` 轴。
- 这是旧版“显式线—面关系”运行时示例。新的全局开放面自动遮挡请看
